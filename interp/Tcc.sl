...
private define link_against_library (s, lib)
{
  ...
  Jit->tcc_set_path (s.tcc, lib, Jit->TCC_ADD_LIB);
}
...
private define add_library_path (s, path)
{
  ...
  Jit->tcc_set_path (s.tcc, path, Jit->TCC_ADD_LPATH);
}
...
private define add_include (s, path)
{
  ...
  Jit->tcc_set_path (s.tcc, path, Jit->TCC_ADD_INC_PATH);
}
...
private define add_include_sys_path (s, path)
{
  ...
  Jit->tcc_set_path (s.tcc, path, Jit->TCC_ADD_SYS_INC_PATH);
}
...
private define add_tcc_path (s, path)
{
  ...
  () = Jit->tcc_set_path (s.tcc, path, Jit->TCC_CONFIG_TCC_DIR);
}
...
private define set_output_type (s, type)
{
  ...
  ifnot (Jit->tcc_set_output_type (s.tcc, type))
    s.output_type = type;
  else
    return -1;
...
private define set_output_file (s, file)
{
  ...
  ifnot (path_is_absolute (file))
    ...
    if (NULL == (p = sysv->realpath (p), p))
      return -1;

    file = path_concat (p, file);
    }
  else
    if (NULL == (p = sysv->realpath (file), p))
      if (NULL == (p = sysv->realpath (path_dirname (file)), p))
        return -1;
      else
  ...
  ifnot (Jit->tcc_set_output_file (s.tcc, file))
    s.output_file = file;
  else
    return -1;
...
private define compile_string (s, cbuf)
{
  ...
  if (isexe)
    ...
    if (-1 == s.set_output_type (Jit->TCC_OUTPUT_EXE))
      return -1;
    }
  else
    if (-1 == s.set_output_type (Jit->TCC_OUTPUT_MEMORY))
      return -1;

  ifnot (0 == Jit->tcc_compile_string (s.tcc, cbuf))
    return -1;
...
private define compile_file (s, file)
{
  ...
  if (-1 == Jit->tcc_compile_file (s.tcc, file))
    return -1;
...
private define cdefine (s, sym, value)
{
  ...
  Jit->tcc_define_symbol (s.tcc, sym, value);
}
...
private define undefine (s, sym)
{
  ...
  Jit->tcc_undefine_symbol (s.tcc, sym);
}
...
private define sladd_intrinsic_variable (s, vname, sym, rettype, cbuf)
{
  ...
  if (-1 == Jit->sladd_variable (s.tcc, vname, sym, rettype;;__qualifiers))
    return -1;
...
private define sladd_intrinsic_function (s, funm, nargs, sym, rettype, argtypes, cbuf)
{
  ...
  if (-1 == Jit->sladd_function (arg_types, s.tcc, funm, sym, rettype, nargs))
    return -1;
...
private define init (self)
{
  variable s = struct
    ...
    output_type = Jit->TCC_OUTPUT_MEMORY,
    iscompiled = 0,
    itwasrun = 0,
    output_file,
    };
